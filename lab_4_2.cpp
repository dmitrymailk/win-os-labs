...
int main()
{
  ...
  равной степени и применимо и к процессом.

  Эта функция позволяет узнать приоритет процесса, а к примеру функция SetThreadPriority
  его назначить\переназначить 
