...
struct print_params // объект параметров для удобства
{
...
int main()
{
  ...
  HINSTANCE hinstLib = LoadLibrary(TEXT("lab_6_2_dll.dll")); // загружаем в runtime нашу библиотеку
  ...
  //  получаем ссылку на нужную функцию из библиотеки
  LPTHREAD_START_ROUTINE thread_1 = (LPTHREAD_START_ROUTINE)GetProcAddress(hinstLib, "thread_1");
  if (!thread_1)
  {
  ...
  LPTHREAD_START_ROUTINE thread_2 = (LPTHREAD_START_ROUTINE)GetProcAddress(hinstLib, "thread_2");
  if (!thread_2)
  {
    cout << "Error import func"
         << GetLastError()
  ...
  // далее идет код идентичный тому что был в 5 лабе
  if (tasks[2])
  {
    params.is_async = 1;
